...
class ExportBitReader {
  ...
  constructor(value: string) {
    this.#values = Array.from(value, (character) =>
      EXPORT_ALPHABET.indexOf(character),
    );
  }
...
function readHeader(value: string): OperationResult<TalentHeader> {
  ...
  return {
    ok: true,
    value: { reader, serializationVersion, specId, treeHash },
    warnings: [],
  };
}
...
export function decodeTalentExportHeader(
  value: string,
): OperationResult<Omit<DecodedTalentLoadout, "talents">> {
...
export function decodeTalentExport(
...
): OperationResult<DecodedTalentLoadout> {
  const header = readHeader(value);
  if (!header.ok) return header;
  const snapshot = snapshots.find(
    (candidate) =>
      ...
      (candidate.treeHash?.toLowerCase() === header.value.treeHash ||
        (candidate.wowVersion !== undefined &&
          candidate.wowVersion === options.wowVersion)),
  );
  ...
  if (snapshot === undefined) {
    return talentFailure(
      ...
      "Update the app's generated production talent data before exporting.",
    );
  }

  const talents: { nodeId: number; entryId: number; rank: number }[] = [];
  for (const node of [...snapshot.nodes].sort(
    ...
    const rank =
      partiallyRanked === 1 ? header.value.reader.read(6) : node.maxRanks;
    ...
    const entryId =
      choiceIndex === undefined ? undefined : node.entryIds[choiceIndex];
    ...
    const tieredNode =
      node.nodeType === 1 ||
      (node.nodeType === undefined && node.entryIds.length > 1);
    if (tieredNode && choiceNode === 0) {
      ...
      for (
        let index = 0;
        index < node.entryIds.length && remaining > 0;
        index += 1
      ) {
