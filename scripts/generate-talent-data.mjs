...
const DEFAULT_OUTPUT = "src/core/combatantInfo/data/generated.ts";
...
const REQUIRED_SPEC_IDS = [
  ...
  269, 270, 577, 581, 1467, 1468, 1473, 1480,
];
...
Usage:
  ...
  npm run talents:update -- --check
  npm run talents:update -- --build 12.1.0.69273
...
async function downloadJson(url, description) {
  const response = await fetch(url, {
    headers: {
      ...
      "user-agent": "wow-log-transformer",
    },
  });
  if (!response.ok) {
    throw new Error(
...
async function loadSource(options) {
  ...
  const [currentRetailBuild, rawMetadata] = await Promise.all([
    ...
    downloadJson(
      `${RAIDBOTS_DATA_BASE_URL}/live/metadata.json`,
      "Raidbots live metadata",
    ),
  ]);
  const metadata = assertObject(rawMetadata, "Raidbots live metadata");
  if (
    requireString(metadata, "environment", "Raidbots live metadata") !== "live"
  ) {
    throw new Error(
      "Raidbots live metadata did not identify the live environment.",
    );
  }
  const dataBuild = requireString(
    metadata,
    "wowBuild",
    "Raidbots live metadata",
  );
...
function packedNode(node, nodeId, specId) {
  ...
  const entries = node.entries
    .map((entry, index) => {
      const entryObject = assertObject(
        entry,
        `${description}.entries[${String(index)}]`,
      );
      ...
      if (!Number.isSafeInteger(maxRanks) || maxRanks <= 0) {
        throw new Error(
          `${description}, entry ${String(id)} has invalid maxRanks.`,
        );
      }
      const indexValue = entryObject.index ?? index * 100;
      if (!Number.isSafeInteger(indexValue)) {
        throw new Error(
          `${description}, entry ${String(id)} has an invalid index.`,
        );
      }
  ...
  if (type === "tiered") {
    return [
      ...
      nodeType,
      entryIds,
      entries.map((entry) => entry.maxRanks),
    ];
  }
...
function buildPackedSpecs(rawTalents) {
  ...
  for (const [index, rawTree] of rawTalents.entries()) {
    ...
    if (bySpec.has(specId)) {
      throw new Error(
        `Raidbots talents.json contains spec ${String(specId)} twice.`,
      );
    }
  ...
  for (const specId of REQUIRED_SPEC_IDS) {
    ...
    if (tree === undefined) {
      throw new Error(
        `Raidbots talents.json is missing required spec ${String(specId)}.`,
      );
    }
    ...
    const nodeOrder = tree.fullNodeOrder.map((nodeId, index) => {
      if (!Number.isSafeInteger(nodeId) || nodeId <= 0) {
        throw new Error(
          `Spec ${String(specId)} fullNodeOrder[${String(index)}] is invalid.`,
        );
      }
      return nodeId;
    });
    if (new Set(nodeOrder).size !== nodeOrder.length) {
      throw new Error(
        `Spec ${String(specId)} fullNodeOrder contains duplicates.`,
      );
    }
    for (let index = 1; index < nodeOrder.length; index += 1) {
      if (nodeOrder[index - 1] >= nodeOrder[index]) {
        throw new Error(
          `Spec ${String(specId)} fullNodeOrder is not ascending.`,
        );
      }
    }
    ...
    for (const collectionName of NODE_COLLECTIONS) {
      ...
      if (!Array.isArray(collection)) {
        throw new Error(
          `Spec ${String(specId)}.${collectionName} is not an array.`,
        );
      }
      for (const rawNode of collection) {
        const node = assertObject(
          rawNode,
          `Spec ${String(specId)}.${collectionName} entry`,
        );
        const nodeId = requireInteger(
          node,
          "id",
          `Spec ${String(specId)}.${collectionName} entry`,
        );
        if (nodes.has(nodeId)) {
          throw new Error(
            `Spec ${String(specId)} repeats node ${String(nodeId)}.`,
          );
        }
...
function renderGeneratedModule(source) {
  ...
  return `// This file is generated by scripts/generate-talent-data.mjs. Do not edit it.
...
async function writeGeneratedFile(output, content, check) {
  ...
  if (check) {
    throw new Error(
      `${path.relative(process.cwd(), absoluteOutput)} is out of date.`,
    );
  }
...
async function main() {
  ...
  const content = renderGeneratedModule(source);
